...
import { Send, Database, AlertTriangle } from "lucide-react";
...
const CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/catalogo-datos-agent`;

export const CatalogoDatosChatAgent = () => {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      recordOperation({ agent: "federated", caseLabel: "Catálogo de Datos", question: text, tokensConsumed: tokensUsed });
    } catch {
  ...
  const { followUps } = lastAssistant ? parseFollowUpMarkers(lastAssistant.content) : { followUps: [] };
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/10 text-primary text-[10px] font-medium">
                <Database className="w-2.5 h-2.5" /> Catálogo
              </span>
            </div>
