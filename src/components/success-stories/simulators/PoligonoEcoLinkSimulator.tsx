...
export const PoligonoEcoLinkSimulator = ({ onValuesChange }: PoligonoEcoLinkSimulatorProps) => {
  ...
  const radarData = useMemo(() => [
    { subject: t('poligonoEcoLink.radar.autoconsumo'), value: calculations.independence },
    { subject: t('poligonoEcoLink.radar.p2p'), value: 60 + numCompanies * 0.8 },
    { subject: t('poligonoEcoLink.radar.storage'), value: 50 + solarSurplus * 0.05 },
    { subject: t('poligonoEcoLink.radar.ai'), value: 75 },
    { subject: t('poligonoEcoLink.radar.resilience'), value: calculations.independence * 0.9 },
      ...
      <div className="lg:col-span-7">
        <Card className="bg-gradient-to-br from-lime-950/40 to-green-950/30 border-lime-500/20 shadow-2xl p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <Badge className="bg-lime-500/20 text-lime-400">{t('poligonoEcoLink.kpi.independence')}: {calculations.independence.toFixed(0)}%</Badge>
          </div>

          <div className="bg-slate-900/80 rounded-2xl p-4 border border-lime-900/30 mb-6">
            <p className="text-xs text-slate-400 uppercase font-bold mb-3 text-center">{t('poligonoEcoLink.radar.title')}</p>
            <div className="h-48">
          ...
          <div className="space-y-5 bg-slate-900/40 p-4 rounded-xl border border-lime-900/20 mb-6">
            <div className="space-y-2">
              <div className="flex justify-between text-sm"><span className="text-slate-300">{t('poligonoEcoLink.slider.surplus')}</span><span className="font-bold text-lime-400">{solarSurplus}</span></div>
              <Slider value={[solarSurplus]} onValueChange={(v) => setSolarSurplus(v[0])} min={10} max={2000} step={10} className="[&>span]:bg-lime-600" />
            </div>
            <div className="space-y-2">
              <div className="flex justify-between text-sm"><span className="text-slate-300">{t('poligonoEcoLink.slider.companies')}</span><span className="font-bold text-green-400">{numCompanies}</span></div>
          ...
          <div className="bg-gradient-to-r from-lime-900/50 to-green-900/50 p-5 rounded-2xl border border-lime-500/30">
            <p className="text-[10px] uppercase font-black text-lime-300 mb-2">{t('poligonoEcoLink.kpi.collectiveSavings')}</p>
      ...
      <div className="lg:col-span-5">
        <Card className="bg-[#020617] border-lime-500/20 shadow-2xl h-full p-6">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-lime-500 to-green-600 flex items-center justify-center"><BrainCircuit className="w-5 h-5 text-white" /></div>
            <div><h4 className="text-white font-bold">{t('aria.name')}</h4><p className="text-[10px] text-slate-400">{t('poligonoEcoLink.aria.role')}</p></div>
          </div>
          ...
          <div className="space-y-4">
            <div className="bg-slate-900/60 rounded-xl p-4 border border-lime-900/30">
              <div className="flex items-start gap-3"><Sparkles className="w-5 h-5 text-lime-400 mt-0.5" />
                <div><p className="text-sm text-white font-medium mb-1">{t('poligonoEcoLink.aria.insight1Title')}</p>
                  <p className="text-xs text-slate-400" dangerouslySetInnerHTML={{ __html: t('poligonoEcoLink.aria.insight1Desc', { percent: calculations.independence.toFixed(0), savings: (calculations.ahorroEuro / numCompanies).toFixed(0) }) }} />
                </div>
              </div>
            </div>
            <div className="bg-slate-900/60 rounded-xl p-4 border border-green-900/30">
              <div className="flex items-start gap-3"><Users className="w-5 h-5 text-green-400 mt-0.5" />
                <div><p className="text-sm text-white font-medium mb-1">{t('poligonoEcoLink.aria.insight2Title')}</p>
                  <p className="text-xs text-slate-400">{t('poligonoEcoLink.aria.insight2Desc', { companies: numCompanies })}</p>
                </div>
              </div>
            </div>
            {calculations.independence >= 70 && (
              <div className="bg-gradient-to-r from-lime-900/30 to-green-900/30 rounded-xl p-4 border border-lime-500/30">
                <div className="flex items-center gap-2 mb-2"><Home className="w-5 h-5 text-lime-400" /><span className="text-sm font-bold text-lime-300">{t('poligonoEcoLink.aria.badgeTitle')}</span></div>
                <p className="text-xs text-slate-300">{t('poligonoEcoLink.aria.badgeDesc')}</p>
              </div>
            )}
          </div>
