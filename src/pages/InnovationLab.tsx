...
const getInnovationLevel = (score: number) => {
  if (score < 40)
    return { label: "Observador", icon: Eye, color: "text-slate-500" };
  if (score < 60)
    return { label: "Explorador", icon: Compass, color: "text-blue-500" };
  if (score < 80)
    return { label: "Pionero", icon: Rocket, color: "text-purple-500" };
...
const CustomScatterTooltip = ({
  ...
  active?: boolean;
  payload?: Array<{
    payload: { name: string; category: string; x: number; y: number };
  }>;
}) => {
...
export default function InnovationLab() {
  ...
  const [selectedConcept, setSelectedConcept] =
    useState<InnovationConcept | null>(null);
  ...
  const DETECTED_ALERTS = [
    {
      ...
      description:
        "Penalización del 15% por retraso > 24h supera el umbral estándar del 5%",
  ...
  const avgCompanyScore =
    TECH_RADAR_DATA.reduce((acc, curr) => acc + curr.company, 0) /
    TECH_RADAR_DATA.length;
  ...
  const simulationData = useMemo(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const newValue =
        previousValue * (1 + growth / 100) + volatility + innovationBoost;
  ...
  if (SHOW_PLACEHOLDER) {
    return (
      <FeaturePlaceholder
        ...
        description="Espacio de experimentación y co-creación de soluciones basadas en datos."
        icon={Lightbulb}
      />
    );
  }
                    ...
                    <Scatter data={scatterData} fill="hsl(var(--primary))">
                      {scatterData.map((entry, index) => (
                        <Cell
                          key={index}
                          fill={
                            CATEGORY_COLORS_HEX[entry.category] ||
                            "hsl(var(--primary))"
                          }
                        />
                      ))}
                ...
                <div className="grid grid-cols-2 gap-2 mt-2 text-xs text-muted-foreground">
                  <div className="text-left">
                    ...
                    <span className="text-green-600 font-medium">
                      {t("prioritization.quickWins")}
                    </span>
                  </div>
                  <div className="text-right">
                    ⬆️{" "}
                    <span className="text-blue-600 font-medium">
                      {t("prioritization.strategic")}
                    </span>
                  </div>
                  <div className="text-left">⬇️ Fill-Ins</div>
                  <div className="text-right">
                    ⬇️ <span className="text-red-600 font-medium">Evitar</span>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          {concepts && concepts.length > 0 && (
            <div className="flex items-center gap-4 text-sm text-muted-foreground pt-4 border-t">
              <span>
                ...
                de{" "}
                <strong className="text-foreground">{concepts.length}</strong>{" "}
                conceptos
              </span>
              {(searchQuery || selectedCategory !== "all") && (
                <Button
            ...
            <Card className="lg:col-span-1 h-fit">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <Slider
                    value={supplyVolatility}
                    onValueChange={setSupplyVolatility}
                    max={100}
                    step={1}
                ...
                <div className="space-y-2">
                  ...
                  <Slider
                    ...
                    onValueChange={setInnovationImpact}
                    max={100}
                    step={1}
                ...
                <div className="pt-4 p-3 bg-muted/30 rounded-lg border text-xs text-muted-foreground">
                  <p>
                    <strong>Nota:</strong> Este modelo utiliza regresión lineal
                    simple con inyección de ruido estocástico basada en los
                    sliders.
                  </p>
                </div>
              </CardContent>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={simulationData}>
                    ...
                    <Area
                      ...
                      name={t("simulator.projection")}
                    />
                    <Area
                      type="monotone"
                      dataKey="limite_seguridad"
                      stroke="#f97316"
                      fill="none"
                      strokeDasharray="5 5"
                      name={t("simulator.safetyLimit")}
                    />
                    <Area
                      type="monotone"
              ...
              <div className="space-y-6">
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          isActive &&
                            "bg-blue-50 dark:bg-blue-950/20 border-blue-300 dark:border-blue-700 ring-2 ring-blue-400/50",
                ...
                <div className="flex justify-center">
                  ...
                  >
                    ...
                    {isSimulating
                      ? t("smartContract.simulating")
                      : t("smartContract.startSimulation")}
                  </Button>
                </div>
              </div>
            </CardContent>
                  ...
                  >
                    ...
                    {scanPhase === "analyzing" && (
                      <>
                        ...
                        {t("sentinel.analyzing")}
                      </>
                    )}
                    {scanPhase === "complete" && (
                      <>
                        <Check className="h-4 w-4" />{" "}
                        {t("sentinel.scanComplete")}
                      </>
                    )}
                  </Button>
            ...
            <CardContent className="h-[500px]">
              ...
              <div className="flex justify-center mt-4">
                <div className="flex items-center gap-2 p-3 bg-muted/50 rounded-lg">
                  <InnovationIcon
                    className={cn("h-5 w-5", innovationLevel.color)}
                  />
                  <span className="font-medium">
                    {t("radar.innovationLevel")}:
                  </span>
                  <Badge variant="secondary">{innovationLevel.label}</Badge>
                  <span className="text-sm text-muted-foreground">
                    ({Math.round(avgCompanyScore)}%)
                  </span>
                </div>
              </div>
            </CardContent>
